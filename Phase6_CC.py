...
def copyRow(county, CountyDataGDB):
    time_start = time.clock()
    if county != "CHAR_51036":
        ROW = os.path.join(CountyDataGDB, county + "_RoW")
        if not arcpy.Exists(ROW):
            arcpy.AddMessage("Moving RoW")
            dir_row = r"\\Ccsvr01\d\GIS\Conservation_Innovation\Classification_Resources\LandUseConversion\VA_GDBMasks"
            gdb_row = os.path.join(dir_row, county + "_Masks.gdb")
            arcpy.AddMessage(gdb_row)
            arcpy.env.workspace = gdb_row
            list_rasters = arcpy.ListRasters()
            for raster in list_rasters:
                if raster == county + "_RoW":
                    arcpy.AddMessage("Copying from " + os.path.join(gdb_row, raster))
                    arcpy.AddMessage("Copying to " + os.path.join(CountyDataGDB, raster))
                    outSetNull = SetNull(os.path.join(gdb_row, raster), "1",
                                         "VALUE = 0")  # If the value == 0, make it null, if the value is != 0, make it 13
                    outSetNull.save(os.path.join(CountyDataGDB, raster))
                    #arcpy.CopyRaster_management(os.path.join(gdb_row, raster),os.path.join(CountyDataGDB, raster))
                    arcpy.AddMessage("That took " + str((time.clock() - time_start) / 60) + " minutes")
        else:
            arcpy.AddMessage("RoW exists already.")

def fixWater(county, CountyDataGDB):
    time_start = time.clock()
    arcpy.AddMessage("Fixing the water for: " + county)

    raster_mosaic = Raster(os.path.join(CountyDataGDB, county + "_Mosaic"))
    raster_orginial = Raster(os.path.join(CountyDataGDB, county + "_LaCoTC"))

    raster_fixed = os.path.join(CountyDataGDB, county + "_Mosaic_water_fix")
    raster_fixed_temp = Con(IsNull(raster_mosaic), (Con(raster_orginial == 11, 4)), raster_mosaic)
    raster_fixed_temp.save(raster_fixed)

    arcpy.AddMessage("That took " + str((time.clock() - time_start) / 60) + " minutes")

def copyCountyRaster(county, CountyDataGDB):
    gdb_counties = r"\\Ccsvr01\d\GIS\Conservation_Innovation\Classification_Resources\LandUseConversion\___Colins_Divvy\County_Shapes.gdb"
    arcpy.env.workspace = gdb_counties

    raster_boundary = "C_" + county[5:]

    list_rasters = arcpy.ListRasters()
    for raster in list_rasters:
        if raster == raster_boundary:
            arcpy.AddMessage("Making " + os.path.join(CountyDataGDB, county + "_Boundary"))
            arcpy.CopyRaster_management(raster, os.path.join(CountyDataGDB, county + "_Boundary"))

def modelOneMeter():
    ###
    ### THIS DOES NOT WORK. IT IS HERE TO GET OUT OF THE WAY.
    ###
    ...
    arcpy.AddMessage("Cleanup " + str((time.clock() - time_start) / 60) + " minutes")
    ...
    if arcpy.Exists(county + "_Herb"):
        ...
        inrasList = []
        if arcpy.Exists(BAR):
            inrasList.append(BAR)
        if arcpy.Exists(LV):
            inrasList.append(LV)
        inrasList = str(";".join(inrasList))  # delimit by ";"
        rasLocation = CountyDataGDB
        arcpy.MosaicToNewRaster_management(inrasList, rasLocation, county + "_Herb", "", "4_BIT", "1", "1", "LAST",
                                           "FIRST")
        HERB = os.path.join(CountyDataGDB, county + "_Herb")
    ...
    if arcpy.Exists(PARCELS):
        ...
        # arcpy.Select_analysis(PARCELS, county + "_Parcels_TURFtemp", 'ACRES2 <= 10')
        # Zstat = ZonalStatisticsAsTable(county + "_Parcels_TURFtemp", "UNIQUEID", INR, "Parcel_IMP", "DATA", "SUM")
        # arcpy.JoinField_management(county + "_Parcels_TURFtemp", "UNIQUEID", "Parcel_IMP", "UNIQUEID", ["SUM"])
        # arcpy.Select_analysis(county + "_Parcels_TURFtemp", county + "_Parcels_TURF", 'SUM >= 93')
        # arcpy.AddField_management(county + "_Parcels_TURF", "VALUE", "SHORT", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")
        # arcpy.CalculateField_management(county + "_Parcels_TURF", "VALUE", "1", "PYTHON_9.3", "")
        # arcpy.FeatureToRaster_conversion(county + "_Parcels_TURF", "VALUE", county + "_TURF_parcels", 1)
        # TURFparcels = os.path.join(str(CountyDataGDB), county + "_TURF_parcels")
        arcpy.Delete_management("in_memory")
        arcpy.AddMessage("Turf 4b took " + str((time.clock() - time_start) / 60) + " minutes")
        ...
        if len(inrasList) > 0:
            rasLocation = CountyDataGDB
            inrasList = str(";".join(inrasList))  # delimit by ";"
            arcpy.MosaicToNewRaster_management(inrasList, rasLocation, county + "_FTGmask", "", "4_BIT", "1", "1",
                                               "LAST", "FIRST")
            arcpy.Delete_management("in_memory")
    ...
    time_start = time.clock()
    if arcpy.Exists(county + "_FTGmask"):
        arcpy.AddMessage("Making fractional turf mask")
    ...
    if not inrasListMO:
        ...
        if arcpy.Exists(BAR):
            if arcpy.Exists(TG):
                BARextra = SetNull(~(IsNull(TG)), BAR)
                BARextra.save(county + "_BarrenExtra")
                BARExtra2 = os.path.join(CountyDataGDB, county + "_BarrenExtra")
                inrasList.append(BARExtra2)
    ...
    if arcpy.Exists(TG):
        LUlist.append(TG)
    if arcpy.Exists(CRP):
        LUlist.append(CRP2)  # use CRP because CRP2 is only defined if CRP exists
    if arcpy.Exists(PAS):
        LUlist.append(PAS2)  # use PAS because PAS2 is only defined if PAS exists
    ...
    arcpy.AddMessage("Final 3 took " + str((time.clock() - time_start) / 60) + " minutes")
    arcpy.AddMessage("********** The whole thing took " + str((time.clock() - all_time_start) / 60) + " minutes ************")
...
for county in list_counties:
    ...
    arcpy.AddMessage("Starting: " + county)
    ...
    #arcpy.env.snapRaster = os.path.join(CountyDataGDB, county + "_LaCoTC")
    #arcpy.env.workspace = CountyDataGDB
    #arcpy.env.scratchWorkspace = Temp1mGBD
    #arcpy.env.extent = os.path.join(CountyDataGDB, county + "_LaCoTC")
    ...
    #copyRow(county, CountyDataGDB)
    #fixWater(county, CountyDataGDB)
    #copyCountyRaster(county, CountyDataGDB)

    ## Environments (10m)
    arcpy.env.workspace = Temp10mGDB
    arcpy.env.scratchWorkspace = Temp10mGDB
    arcpy.env.extent = os.path.join(CountyDataGDB, county + "_Boundary")
    arcpy.env.snapRaster = os.path.join(CountyDataGDB, county + "_Boundary")
    coord_data = os.path.join(CountyDataGDB, county + "_Boundary")
    arcpy.env.outputCoordinateSystem = arcpy.Describe(coord_data).spatialReference

    # Final 4: Split Mosaic and Reclass Land Uses to [1,0]
    time_start = time.clock()

    arcpy.Delete_management(os.path.join(TempDirectory, county + "_LuTable.dbf"))
    arcpy.TableToTable_conversion(os.path.join(CountyDataGDB, county + "_Mosaic_water_fix"), TempDirectory, county + "_LuTable.dbf")
    LuTable = os.path.join(TempDirectory, county + "_LuTable.dbf")
    arcpy.JoinField_management(LuTable, "Value", os.path.join(dir_landuse, "ClassNames.dbf"), "Value", ["Class_Name"])
    ...
    for row in rows:
        ...
        rasExtract = ExtractByAttributes(os.path.join(CountyDataGDB, county + "_Mosaic_water_fix"), sqlQuery)
        outCon = Con(IsNull(rasExtract), 0, 1)
        outAgg = Aggregate(outCon, 10, "SUM", "TRUNCATE", "DATA")
        outAgg.save(os.path.join(Temp10mGDB, county + "_" + str(luAbr) + "_10m"))
    #outCon2 = Con(IsNull(  os.path.join(CountyDataGDB, county + "_Mosaic_water_fix")), 1, 0)
    #outAgg2 = Aggregate(outCon2, 10, "SUM", "TRUNCATE", "DATA")
    #outAgg2.save(os.path.join(Output10mGDB, county + "_" + "AG_10m"))

    arcpy.AddMessage("Final 4 took " + str((time.clock() - time_start) / 60) + " minutes")

    # Final 5: Combine all 10m rasters (8-10 minutes)
    time_start = time.clock()

    IR = os.path.join(Temp10mGDB, county + "_IR_10m")
    INR = os.path.join(Temp10mGDB, county + "_INR_10m")
    TCI = os.path.join(Temp10mGDB, county + "_TCI_10m")
    WAT = os.path.join(Temp10mGDB, county + "_WAT_10m")
    WLT = os.path.join(Temp10mGDB, county + "_WLT_10m")
    WLF = os.path.join(Temp10mGDB, county + "_WLF_10m")
    WLO = os.path.join(Temp10mGDB, county + "_WLO_10m")
    FOR = os.path.join(Temp10mGDB, county + "_FOR_10m")
    TCT = os.path.join(Temp10mGDB, county + "_TCT_10m")
    MO = os.path.join(Temp10mGDB, county + "_MO_10m")
    FTG = os.path.join(Temp10mGDB, county + "_FTG_10m")
    FINR = os.path.join(Temp10mGDB, county + "_FINR_10m")
    TG = os.path.join(Temp10mGDB, county + "_TG_10m")
    CRP = os.path.join(Temp10mGDB, county + "_CRP_10m")
    PAS = os.path.join(Temp10mGDB, county + "_PAS_10m")

    DEMstrm = os.path.join(ResourceGDB, "Stream_10m")
    outCon = Con(IsNull(DEMstrm), 0, DEMstrm)
    outCon.save(os.path.join(Temp10mGDB, county + "_NewStrm"))
    NewStrm = os.path.join(Temp10mGDB, county + "_NewStrm")

    #AG = os.path.join(Output10mGDB, county + "_AG_10m")
    ...
    outCombine.save(os.path.join(Temp10mGDB, county + "_Combo"))
    arcpy.Delete_management("in_memory")
    ...
    arcpy.TableToTable_conversion(os.path.join(Temp10mGDB, county + "_Combo"), Temp10mGDB, "Combo")
    ...
    arcpy.CalculateField_management(Combo, "CRP_1",
                                    "(!AG! - (((!AG! * !crpCD!)/100) + ((!AG! * !pasCD!)/100)))*0.5 + (!AG! * !crpCD!)/100",
                                    "PYTHON_9.3")
    arcpy.CalculateField_management(Combo, "PAS_1",
                                    "(!AG! - (((!AG! * !pasCD!)/100) + ((!AG! * !crpCD!)/100)))*0.5 + (!AG! * !pasCD!)/100",
                                    "PYTHON_9.3")
    if arcpy.Exists(WLT) and arcpy.Exists(WLF):
        arcpy.CalculateField_management(Combo, "DIFF_1",
                                        "abs(100 - (!IR!+ !INR_1!+ !WAT_1! + !WLT! + !WLF! + !WLO! + !FOR_! + !TCI! + !TCT! + !MO_1! + !TG_1! + !CRP_1! + !PAS_1!))",
                                        "PYTHON_9.3")
        arcpy.CalculateField_management(Combo, "NAT",
                                        "(!WLT! + !WLF! + !WLO! + !FOR_! + !TCT! + !MO_1! + !TG_1! + !CRP_1! + !PAS_1!)",
                                        "PYTHON_9.3")
    elif arcpy.Exists(WLF):
        arcpy.CalculateField_management(Combo, "DIFF_1",
                                        "abs(100 - (!IR!+ !INR_1!+ !WAT_1! + !WLF! + !WLO! + !FOR_! + !TCI! + !TCT! + !MO_1! + !TG_1! + !CRP_1! + !PAS_1!))",
                                        "PYTHON_9.3")
        arcpy.CalculateField_management(Combo, "NAT",
                                        "(!WLF! + !WLO! + !FOR_! + !TCT! + !MO_1! + !TG_1! + !CRP_1! + !PAS_1!)",
                                        "PYTHON_9.3")
    else:
        arcpy.CalculateField_management(Combo, "DIFF_1",
                                        "abs(100 - (!IR!+ !INR_1!+ !WAT_1! + !WLO! + !FOR_! + !TCI! + !TCT! + !MO_1! + !TG_1! + !CRP_1! + !PAS_1!))",
                                        "PYTHON_9.3")
        arcpy.CalculateField_management(Combo, "NAT", "(!WLO! + !FOR_! + !TCT! + !MO_1! + !TG_1! + !CRP_1! + !PAS_1!)",
                                        "PYTHON_9.3")
    ...
    arcpy.CalculateField_management("Subset", "WAT_2", "(!WAT_1! + (!NAT! - !DIFF_1!))",
                                    "PYTHON_9.3")  # max of WAT and WAT_1
    if arcpy.Exists(WLT):
    ...
    if arcpy.Exists(WLT) and arcpy.Exists(WLF):
        arcpy.CalculateField_management("Subset", "DIFF_2",
                                        "(100 - (!IR! + !INR_1! + !WAT_2! + !WLT_1! + !WLF_1! + !WLO_1! + !FOR_1! + !TCI! + !TCT_1! + !MO_2! + !TG_2! + !CRP_2! + !PAS_2!))",
                                        "PYTHON_9.3")
    elif arcpy.Exists(WLF):
        arcpy.CalculateField_management("Subset", "DIFF_2",
                                        "(100 - (!IR! + !INR_1! + !WAT_2! + !WLF_1! + !WLO_1! + !FOR_1! + !TCI! + !TCT_1! + !MO_2! + !TG_2! + !CRP_2! + !PAS_2!))",
                                        "PYTHON_9.3")
    else:
        arcpy.CalculateField_management("Subset", "DIFF_2",
                                        "(100 - (!IR! + !INR_1! + !WAT_2! + !WLO_1! + !FOR_1! + !TCI! + !TCT_1! + !MO_2! + !TG_2! + !CRP_2! + !PAS_2!))",
                                        "PYTHON_9.3")
    ...
    arcpy.MakeRasterLayer_management(os.path.join(Temp10mGDB, county + "_Combo", "Adjusted"))
    arcpy.AddJoin_management("Adjusted", "Value", Combo, "Value")  # no need to save Combo as a DBF prior to this.
    P6classes = arcpy.sa.ExtractByMask("Adjusted",
                                       "Adjusted")  # Fooling ArcGIS to convert the Raster Layer to Raster Dataset (must include sa in the string).
    arcpy.Delete_management("in_memory")
    outRaster1 = Lookup(P6classes, "IR")
    outRaster1.save(os.path.join(Output10mGDB, county + "_IR"))
    outRaster2 = Lookup(P6classes, "INR_1")
    outRaster2.save(os.path.join(Output10mGDB, county + "_INR"))
    outRaster3 = Lookup(P6classes, "TCI")
    outRaster3.save(os.path.join(Output10mGDB, county + "_TCI"))
    outRaster4 = Lookup(P6classes, "WAT_2")
    outRaster4.save(os.path.join(Output10mGDB, county + "_WAT"))
    if arcpy.Exists(WLT):
        outRaster5 = Lookup(P6classes, "WLT_1")
        outRaster5.save(os.path.join(Output10mGDB, county + "_WLT"))
    if arcpy.Exists(WLF):
        outRaster6 = Lookup(P6classes, "WLF_1")
        outRaster6.save(os.path.join(Output10mGDB, county + "_WLF"))
    outRaster7 = Lookup(P6classes, "WLO_1")
    outRaster7.save(os.path.join(Output10mGDB, county + "_WLO"))
    outRaster8 = Lookup(P6classes, "FOR_1")
    outRaster8.save(os.path.join(Output10mGDB, county + "_FOR"))
    outRaster9 = Lookup(P6classes, "TCT_1")
    outRaster9.save(os.path.join(Output10mGDB, county + "_TCT"))
    outRaster10 = Lookup(P6classes, "MO_3")
    outRaster10.save(os.path.join(Output10mGDB, county + "_MO"))
    outRaster11 = Lookup(P6classes, "TG_3")
    outRaster11.save(os.path.join(Output10mGDB, county + "_TG"))
    outRaster12 = Lookup(P6classes, "CRP_2")
    outRaster12.save(os.path.join(Output10mGDB, county + "_CRP"))
    outRaster13 = Lookup(P6classes, "PAS_2")
    outRaster13.save(os.path.join(Output10mGDB, county + "_PAS"))

    arcpy.env.workspace = Output10mGDB

    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_IR"), os.path.join(TifDirectory, county + "_IR.tif"), "",
                                "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_INR"), os.path.join(TifDirectory, county + "_INR.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_TCI"), os.path.join(TifDirectory, county + "_TCI.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_WAT"), os.path.join(TifDirectory, county + "_WAT.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    if arcpy.Exists(WLT):
        arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_WLT"),
                                    os.path.join(TifDirectory, county + "_WLT.tif"), "", "0", "0", "NONE", "NONE",
                                    "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    if arcpy.Exists(WLF):
        arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_WLF"),
                                    os.path.join(TifDirectory, county + "_WLF.tif"), "", "0", "0", "NONE", "NONE",
                                    "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_WLO"), os.path.join(TifDirectory, county + "_WLO.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_FOR"), os.path.join(TifDirectory, county + "_FOR.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_TCT"), os.path.join(TifDirectory, county + "_TCT.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_MO"), os.path.join(TifDirectory, county + "_MO.tif"), "",
                                "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_TG"), os.path.join(TifDirectory, county + "_TG.tif"), "",
                                "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_CRP"), os.path.join(TifDirectory, county + "_CRP.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
    arcpy.CopyRaster_management(os.path.join(Output10mGDB, county + "_PAS"), os.path.join(TifDirectory, county + "_PAS.tif"),
                                "", "0", "0", "NONE", "NONE", "8_BIT_UNSIGNED", "NONE", "NONE", "TIFF")
